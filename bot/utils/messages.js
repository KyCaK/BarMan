/**
 * Шаблоны сообщений для бота
 */

export const MESSAGES = {
  // Приветствие
  WELCOME: (firstName) => `
👋 Привет, ${firstName}!

Добро пожаловать в магазин натуральных соков!

🍊 Свежие соки из натуральных фруктов и овощей
🚀 Быстрая доставка по городу
💚 100% натуральные ингредиенты
⭐ Отличное качество по доступным ценам

Нажмите кнопку ниже, чтобы открыть каталог и сделать заказ:
`,

  // Команды помощи
  HELP: `
🤖 *Доступные команды:*

🍹 *Основные:*
• Используйте кнопку "Открыть меню" для просмотра каталога
• 📋 Мои заказы - история ваших заказов
• 📞 Контакты - наши контакты

👨‍💻 *Для админов:*
• /admin - панель администратора
• /stats - статистика заказов
• /users - список пользователей

❓ *Нужна помощь?*
Напишите нам в любое время, мы всегда готовы помочь!
`,

  // Контакты
  CONTACTS: `
📞 *Контактная информация*

📱 *Телефон:* +998 90 123 45 67
📧 *Email:* info@juices.uz
🕐 *Режим работы:* Ежедневно 9:00 - 21:00

📍 *Адрес:* 
г. Ташкент, ул. Примерная, дом 1

🚚 *Доставка:*
• По городу: 15,000 сум
• При заказе от 100,000 сум - доставка бесплатно
• Время доставки: 30-60 минут

💬 Пишите нам в любое время!
`,

  // О компании
  ABOUT: `
🍹 *О нас*

Мы - команда энтузиастов здорового питания, которая создает натуральные соки из отборных фруктов и овощей.

🌱 *Наши принципы:*
• Только свежие и качественные ингредиенты
• Никаких консервантов и искусственных добавок
• Современные технологии приготовления
• Строгий контроль качества

🏆 *Почему выбирают нас:*
• Более 50 видов соков
• Быстрая доставка
• Доступные цены
• Отличные отзывы клиентов

Спасибо, что выбираете нас! 💚
`,

  // Ошибки
  ERRORS: {
    GENERAL: 'Произошла ошибка. Попробуйте позже.',
    NO_ORDERS: 'У вас пока нет заказов.',
    ORDER_NOT_FOUND: 'Заказ не найден.',
    ACCESS_DENIED: 'У вас нет доступа к этой функции.',
    INVALID_COMMAND: 'Неизвестная команда. Используйте /help для просмотра доступных команд.'
  },

  // Заказы
  ORDER: {
    CONFIRMED: (orderNumber) => `✅ Ваш заказ №${orderNumber} подтвержден!\nМы свяжемся с вами в ближайшее время.`,
    PREPARING: (orderNumber) => `👨‍🍳 Ваш заказ №${orderNumber} готовится!\nСовсем скоро будет готов.`,
    DELIVERING: (orderNumber) => `🚚 Ваш заказ №${orderNumber} передан курьеру!\nОжидайте доставку в течение 30-60 минут.`,
    COMPLETED: (orderNumber) => `✨ Ваш заказ №${orderNumber} выполнен!\nСпасибо за покупку! Будем рады видеть вас снова.`,
    CANCELLED: (orderNumber) => `❌ К сожалению, ваш заказ №${orderNumber} отменен.\nПо всем вопросам обращайтесь к нашим операторам.`
  },

  // Админ
  ADMIN: {
    WELCOME: 'Добро пожаловать в панель администратора',
    NO_ORDERS: 'Новых заказов нет',
    NO_STATS: 'Статистика пока недоступна'
  }
}

/**
 * Форматирует заказ для отображения
 */
export const formatOrder = (order) => {
  const statusEmoji = {
    pending: '⏳',
    confirmed: '✅', 
    preparing: '👨‍🍳',
    delivering: '🚚',
    completed: '✨',
    cancelled: '❌'
  }

  const statusText = {
    pending: 'Ожидает подтверждения',
    confirmed: 'Подтвержден',
    preparing: 'Готовится', 
    delivering: 'В доставке',
    completed: 'Выполнен',
    cancelled: 'Отменен'
  }

  const date = new Date(order.created_at).toLocaleDateString('ru-RU', {
    day: 'numeric',
    month: 'short',
    hour: '2-digit',
    minute: '2-digit'
  })

  let message = `${statusEmoji[order.status]} *Заказ №${order.order_number}*\n`
  message += `📅 ${date}\n`
  message += `💰 ${order.total_amount} сум\n`
  message += `📍 ${statusText[order.status]}\n`

  if (order.order_items && order.order_items.length > 0) {
    message += `\n🍹 *Состав заказа:*\n`
    order.order_items.forEach((item) => {
      message += `  • ${item.juice_name} x${item.quantity}\n`
    })
  }

  if (order.comment) {
    message += `\n💬 *Комментарий:* ${order.comment}\n`
  }

  if (order.phone_number) {
    message += `📞 ${order.phone_number}\n`
  }

  return message
}

/**
 * Форматирует статистику
 */
export const formatStats = (stats) => {
  return `📊 *Статистика*

*Сегодня:*
📦 Заказов: ${stats.today.count}
💰 Выручка: ${stats.today.revenue.toFixed(0)} сум

*За неделю:*
📦 Заказов: ${stats.week.count}
💰 Выручка: ${stats.week.revenue.toFixed(0)} сум

*За месяц:*
📦 Заказов: ${stats.month.count}
💰 Выручка: ${stats.month.revenue.toFixed(0)} сум

*Всего:*
📦 Всего заказов: ${stats.total.count}
👥 Пользователей: ${stats.total.users}
`
}

/**
 * Экранирует спецсимволы Markdown
 */
export const escapeMarkdown = (text) => {
  return text.replace(/[_*\[\]()~`>#+=|{}.!-]/g, '\\$&')
}